# install.ps1 -- Lumos slim edition, Windows thin shell. Forwards everything to install.py.
#
...
function Invoke-Install {
  ...
  & $Py.Source "$Pkg\install.py" @ScriptArgs | Out-Host
  return $LASTEXITCODE
}

$Pkg = Split-Path -Parent $MyInvocation.MyCommand.Path
$rc = Invoke-Install -Pkg $Pkg -ScriptArgs $args
